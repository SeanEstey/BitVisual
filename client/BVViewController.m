...
-(void)initialize
{
    getMarketsURL = @"http://www.seanestey.ca/bitvisual/python/get_markets.py";
    ...
    currenciesArray = [NSMutableArray arrayWithObjects:
                       @"USD",
                       @"CAD",
                       @"GBP",
                       @"CNY",
    ...
    marketDataTimer = [NSTimer scheduledTimerWithTimeInterval:30
                      target:self
                      selector:@selector(checkTickerTimer:)
                      userInfo:nil
                      repeats:YES];
}
...
-(void)loadView
{
    ...
    [retryConnectionButton addTarget:self action:@selector(retryConnection:)
                           forControlEvents:UIControlEventTouchUpInside];
    retryConnectionButton.titleLabel.font = [UIFont fontWithName:@"Futura" size:18];
    [retryConnectionButton setTitleColor:[UIColor blackColor]
    ...
    [retryConnectionButton setTitle:@"Retry"
                           forState:UIControlStateNormal];
    ...
    // EXCHANGE BTN
    exchangeButton = [[UIButton alloc] initWithFrame:CGRectMake(
                      screenBounds.size.width/2-exchangeButWidth/2,
                      5,
                      exchangeButWidth,
                      40)];
    [exchangeButton addTarget:self action:@selector(toggleExchangesMenu:)
                    forControlEvents:UIControlEventTouchUpInside];
    ...
    if(exchangeSymbols[exchange])
        [exchangeButton setTitle:[NSString stringWithFormat:@"%@ %C", exchangeSymbols[exchange], (unichar)0x25BC]
                        forState:UIControlStateNormal];
    else
        [exchangeButton setTitle:[NSString stringWithFormat:@"%@ %C", exchange, (unichar)0x25BC]
                        forState:UIControlStateNormal];
    ...
    [currencyButton addTarget:self action:@selector(toggleCurrenciesMenu:)
                    forControlEvents:UIControlEventTouchUpInside];
    currencyButton.frame = CGRectMake(
                            priceLabel.frame.origin.x + priceLabel.frame.size.width,
                            priceLabel.frame.origin.y + 15,
                            65,
                            50);
    currencyButton.titleLabel.font = [UIFont fontWithName:@"Futura" size:18];
    [currencyButton setTitleColor:[UIColor whiteColor]
                    forState:UIControlStateNormal];
    ...
    [themesButton setBackgroundImage:[UIImage imageNamed:@"gear_black_bg.png"]
                  forState:UIControlStateNormal];
    [themesButton addTarget:self action:@selector(toggleThemesMenu:)
                  forControlEvents:UIControlEventTouchUpInside];
    themesButton.frame = CGRectMake(5,5,35,35);
    [themesButton setTitleColor:[UIColor whiteColor]
                  forState:UIControlStateNormal];
    [self.view addSubview:themesButton];

    // TIMESPAN BTN
    timeSpanButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    [timeSpanButton addTarget:self action:@selector(toggleTimeSpanMenu:)
                    forControlEvents:UIControlEventTouchUpInside];
    timeSpanButton.frame = CGRectMake(
                           (self.view.frame.size.width / 2.0) - (timeSpanBtnWidth / 2.0),
                           screenBounds.size.height - 100,
                           timeSpanBtnWidth,
                           timeSpanBtnHeight);
    timeSpanButton.titleLabel.font = [UIFont fontWithName:@"Futura" size:18];
    [timeSpanButton setTitleColor:[UIColor whiteColor]
                    forState:UIControlStateNormal];
    ...
    UITapGestureRecognizer *priceLblGR = [[UITapGestureRecognizer alloc]
                                          initWithTarget:self
                                          action:@selector(tapPriceLabel:)];
    priceLblGR.numberOfTapsRequired = 1;
    priceLabel.userInteractionEnabled = TRUE;
    [priceLabel addGestureRecognizer:priceLblGR];
    [self.view insertSubview:priceLabel
            belowSubview:exchangeButton];
    
    // TIME LBL
    timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(
                screenBounds.size.width/2-timeLabelWidth/2,
                105,
                timeLabelWidth,
                25)];
    ...
    // PRICE CHANGE LBL
    priceChangeLabel = [[UILabel alloc]initWithFrame:CGRectMake(
                        screenBounds.size.width - 50,
                        currencyButton.frame.origin.y+30,
                        65,
                        50)];
    ...
    CGRect chartFrame = CGRectMake(
                        0,
                        screenBounds.origin.y+(screenBounds.size.height*0.30f),
                        screenBounds.size.width,
                        screenBounds.size.height*0.70f);
    ...
    toolbar.hidden = YES;
    UITapGestureRecognizer *singleTapGR = [[UITapGestureRecognizer alloc]
                                           initWithTarget:self
                                           action:@selector(escapeMenu:)];
    singleTapGR.numberOfTapsRequired = 1;
    [toolbar addGestureRecognizer:singleTapGR];
    [self.view addSubview:toolbar];
    
    // PICKER
    self.pickerView = [[UIPickerView alloc]initWithFrame:CGRectMake(
                       0,
                       screenBounds.size.height - pickerHeight,
                       screenBounds.size.width,
                       pickerHeight)];
...
{
    if(exchangeSymbols[exchange])
        [exchangeButton setTitle:[NSString stringWithFormat:@"%@", exchangeSymbols[exchange]]
                        forState:UIControlStateNormal];
    else
        [exchangeButton setTitle:[NSString stringWithFormat:@"%@", exchange]
                        forState:UIControlStateNormal];
    ...
    priceLabel.frame = CGRectMake(
                        (self.view.frame.size.width / 2.0) - (priceLabel.frame.size.width / 2.0),
    ...
    [currencyButton setTitle:[NSString stringWithFormat:@"%@/BTC",currencyUnicodes[self.currency]]
                    forState:UIControlStateNormal];
    r.origin.y+=30;
...
-(void)checkGraphTimer
{
    ...
    NSDateComponents* lastRightEdgeUpdateComp = [calendar components: NSMinuteCalendarUnit | NSHourCalendarUnit
                                                            fromDate:priceGraph.lastRightEdgeUpdate];
    NSDate* nowDate = [NSDate date];
    NSDateComponents* now = [calendar components: NSMinuteCalendarUnit | NSHourCalendarUnit
                                      fromDate:nowDate];
...
-(void)getMarketPrices:(bool)in_background
{
    ...
    NSURLRequest* r1 = [NSURLRequest requestWithURL:[NSURL URLWithString:getMarketsURL]];
    ((ConnectionData*)connections[MARKET_DATA]).data = [NSMutableData data];
    ((ConnectionData*)connections[MARKET_DATA]).connection = [[NSURLConnection alloc] initWithRequest:r1
                                                                                      delegate:self];
}
...
-(void)connectionDidFinishLoading:(NSURLConnection *)c
{
    ...
    {
	    NSString *responseString = [[NSString alloc] initWithData:((ConnectionData*)connections[MARKET_DATA]).data
                                    encoding:NSUTF8StringEncoding];
		NSError *e = nil;
		NSData *jsonData = [responseString dataUsingEncoding:NSUTF8StringEncoding];
        NSDictionary* jsonDictData = [NSJSONSerialization JSONObjectWithData:jsonData
...
-(IBAction)toggleExchangesMenu:(id)sender
{
    ...
    if(exchangesMenu == nil)
    {
        ...
        exchangesMenu = [[BVMenu alloc]
                         showDropDown:sender
                         hasWidth:width
                         hasHeight:height
                         hasItems:exchangesArray
                         direction:@"down"
                         editsSenderOnSelect:TRUE
                         caption:exchangeSymbols[exchange]];
...
-(IBAction)toggleThemesMenu:(id)sender
{
    ...
    if(themesMenu == nil)
    {
        ...
        themesMenu = [[BVMenu alloc]showDropDown:sender
                        hasWidth:width
                        hasHeight:height
                        hasItems:themeFilenames
                        direction:@"down"
                        editsSenderOnSelect:false caption:@"Theme"];
...
-(IBAction)toggleCurrenciesMenu:(id)sender
{
    ...
    if(currenciesMenu == nil)
    {
        ...
        currenciesMenu = [[BVMenu alloc]showDropDown:sender
                          hasWidth:width
                          hasHeight:height
                          hasItems:currencies
                          direction:@"down"
                          editsSenderOnSelect:FALSE
                          caption:self.currency];
